...
#     # parser.add_argument(choices=['mini-MIAS', 'mini-MIAS-binary', 'CBIS-DDMS', 'CMMD', 'INbreast'], required=True, help="The dataset to use.")
...
#     # choices=["mini-MIAS","mini-MIAS-binary","CBIS-DDMS"],
#     # help="The dataset to use.")
#     parser.add_argument(
#         "-d", "--dataset",
#         choices=["mini-MIAS", "mini-MIAS-binary", "CBIS-DDMS", "CMMD", "INbreast"],
#         required=True,
#         help="mini-MIAS, mini-MIAS-binary, CBIS-DDSM, CMMD, INbreast"
#     )
#     parser.add_argument("-mt", "--mammogram_type",
#                         choices=["calc","mass","all"], default="all",
#                         help="The mammogram type to use.")
#     parser.add_argument("-m", "--model", required=True,
#                         choices=["CNN","VGG","VGG-common","ResNet","Inception","DenseNet","MobileNet"],
#                         help="Model to use.")
#     parser.add_argument("-r", "--runmode", default="train",
#                         choices=["train","test"],
#                         help="train or test")
#     parser.add_argument("-lr","--learning_rate", type=float, default=0.001)
#     parser.add_argument("-b","--batch_size", type=int, default=2)
...
#     # --- Thêm branch cho INbreast ---
#     elif config.dataset.upper() == "INBREAST":
#         data_dir = os.path.join("..","data","INbreast")
#         # import_inbreast_dataset sẽ load ROI hoặc toàn ảnh theo config.is_roi :contentReference[oaicite:0]{index=0}&#8203;:contentReference[oaicite:1]{index=1}
#         X, y = data_preprocessing.import_inbreast_dataset(data_dir, label_encoder)
#         X_train, X_test, y_train, y_test = data_preprocessing.dataset_stratified_split(0.2, X, y)
#         if getattr(config, "augment_data", False):
#             X_train, y_train = data_transformations.generate_image_transforms(X_train, y_train)
...
def main():
    ...
    if config.dataset in ["mini-MIAS","mini-MIAS-binary"]:
        d = os.path.join(PROJECT_ROOT, "data", config.dataset)
        ...
        d = os.path.join(PROJECT_ROOT, "data", "CMMD-binary")
    ...
    #     d = os.path.join(PROJECT_ROOT, "data", "INbreast")
