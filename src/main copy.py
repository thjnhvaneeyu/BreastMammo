...
from data_operations.data_preprocessing import calculate_class_weights, dataset_stratified_split, \
    import_cbisddsm_testing_dataset, import_cbisddsm_training_dataset, import_minimias_dataset, import_cmmd_dataset
...
def main() -> None:
    ...
    if config.run_mode == "train":
        ...
        if config.dataset == "mini-MIAS":
            ...
            # Import entire dataset.
            images, labels = import_cmmd_dataset(data_dir="../data/{}/images_processed".format(config.dataset),
                                                 label_encoder=l_e)
            ...
            X_train, X_val, y_train, y_val = dataset_stratified_split(split=0.25,
                                                                      dataset=X_train,
                                                                      labels=y_train)
            # Tính toán class_weights
            class_weights = calculate_class_weights(y_train, l_e)
            print("Class weights:", class_weights)
            if class_weights is None:
    # Tạo trọng số mặc định dựa trên tỷ lệ phân bố lớp
            ...
            # Import entire dataset.
            print("Processing INbreast dataset...")
            images, labels = import_inbreast_dataset(data_dir="../data/{}/images_processed".format(config.dataset),
                                                     label_encoder=l_e)
            class_weights = calculate_class_weights(y_train, l_e)
            # Split dataset into training/test/validation sets (80/20% split).
            X_train, X_test, y_train, y_test = dataset_stratified_split(split=0.20, dataset=images, labels=labels)
    ...
    # Run in testing mode.
        ...
        # Test binary classification (binarised mini-MIAS dataset).
            ...
            predictions = model.predict(x=X_test)
            runtime = round(time.time() - start_time, 2)
            test_model_evaluation(y_test, predictions, l_e, 'B-M', runtime)        
